...
<footer class="Footer">
    <div class="container">
        ...
        <div class="Footer-content">
            <div class="Footer-left">
                <div class="Footer-contact">
                    <div class="Footer-address"><?= get_field('address', 'options') ?></div>
                    <div class="Footer-facebook">
                        Suivez-nous sur
                        <a href="<?= get_field('facebook_link', 'options')['url'] ?>"
                           target="<?= get_field('facebook_link', 'options')['target'] ?>">Facebook</a>
                    </div>
                </div>
                <div class="Footer-buttons">
                    <a href="<?= $buttons[' route']['url'] ?>" target="<?= $buttons['route']['target'] ?>"
                       class="Button isBorderRadius isMedium isSecondaryLight">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20.238" height="20.238"
                             viewBox="0 0 20.238 20.238">
                            <g id="icone-itineraire" transform="translate(0 0.001)">
                                <g id="directions" transform="translate(0 -0.001)">
                                    <path id="Tracé_882" data-name="Tracé 882"
                                          d="M19.935,9.41,10.827.3h0A.978.978,0,0,0,9.411.3L.3,9.41a.978.978,0,0,0,0,1.417l9.107,9.107h0a.978.978,0,0,0,1.417,0l9.107-9.107A.978.978,0,0,0,19.935,9.41Zm-7.792,3.238v-2.53H8.1v3.036H6.071V9.106A.956.956,0,0,1,7.083,8.094h5.06V5.565l3.542,3.542Z"
                                          transform="translate(0 0.001)" fill="#00497e"/>
                                </g>
                            </g>
                        </svg>
                        <?= $buttons['route']['title'] ?>
                    </a>
                    <a href="<?= $buttons[' email_address']['url'] ?>"
                       target="<?= $buttons['email_address']['target'] ?>"
                       class="Button isMedium isBorderRadius isSecondaryLight">
                    ...
                    <a href="<?= $buttons[' telephone']['url'] ?>" target="<?= $buttons['telephone']['target'] ?>"
                       class="Button isMedium isBorderRadius isSecondaryLight">
            ...
            <div class="Footer-right">
                <?php if (count($primaryUsefulLinks) > 0) : ?>
                    <div class="Footer-usefulLinks">
                        <?php foreach ($primaryUsefulLinks as $primaryUsefulLink) : ?>
                            <a href="<?= $primaryUsefulLink[' link']['url'] ?>" target="<?= $primaryUsefulLink['link']['target'] ?>">
                                <?= $primaryUsefulLink['link']['title'] ?>
                            </a>
                        <?php endforeach; ?>
                    </div>
                <?php endif; ?>

                <?php if (count($secondaryUsefulLinks) > 1) : ?>
                <div class="Footer-row">
                    <div class="Footer-usefulLinks">
                        <?php foreach ($leftLinks as $leftLink) : ?>
                            <a href="<?= $leftLink[' link']['url'] ?>" target="<?= $leftLink['link']['target'] ?>">
                                <?= $leftLink['link']['title'] ?>
                            </a>
                        <?php endforeach; ?>
                    </div>
                    ...
                    <div class=" Footer-usefulLinks">
                        <?php foreach ($rightLinks as $rightLink) : ?>
                            <a href="<?= $rightLink[' link']['url'] ?>" target="<?= $rightLink['link']['target'] ?>">
                                <?= $rightLink['link']['title'] ?>
                            </a>
                        <?php endforeach ?>
                    </div>
                    <?php endif; ?>
        ...
        <div class="Footer-foot">
            <?php if (count($footLinks) > 0) : ?>
                <?php foreach ($footLinks as $footLink) : ?>
                    <a href="<?= $footLink[' link']['url'] ?>" target="<?= $footLink['link']['target'] ?>">
                        <?= $footLink['link']['title'] ?>
                    </a>
                <?php endforeach; ?>
            <?php endif; ?>
